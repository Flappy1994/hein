	...
	$.extend(
		...
		$.fbuilder.controls[ 'ffields' ].prototype,
		{
			title:"Appointment",
			ftype:"fapp",			
			services:new Array({name:"Sample Service",price:24.99,capacity:1,duration:60,pb:0,pa:0}),
            ...
            var me = this;
					  ...
					  $(".services .choice_add").bind("click", {obj: this}, function(e) 
						{
							e.data.obj.services.splice($(this).attr("i")*1+1,0,{name:"Sample Service",price:24.99,capacity:1,duration:60,pb:0,pa:0});
							$.fbuilder.editItem(e.data.obj.index);
							$.fbuilder.reloadItems({'field':e.data.obj});
						});
			...
			after_show:function()
			  {
			  	  function onChangeDateOrService(d)
			  	  {
			  	  	  function formattime(t)
								...
								var st, et, str = "";
								var pa = me.services[$(".fieldCalendarService"+me.name+" select option:selected").index()].pa * 1 || 0;								
								var pb = me.services[$(".fieldCalendarService"+me.name+" select option:selected").index()].pb * 1 || 0;
								if (arr[d])
