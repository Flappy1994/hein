    ...
    function widget($args, $instance) 
	{
		...
		<ul class="social_icons clearfix">
			...
			for($i=0; $i<count($icon_type); $i++)
			{
				if($icon_type[$i]!=""):
			?>
			<li><a<?php echo ($icon_target[$i]=="new_window" ? " target='_blank'" : ""); ?> href="<?php echo $icon_value[$i];?>" class="social_icon" style="background-image: url('<?php echo get_stylesheet_directory_uri() . "/images/social_footer/" . $icon_color[$i] . "/" . $icon_type[$i] . ".png');"; ?>"></a></li>
			<?php
				endif;
			}
			?>
