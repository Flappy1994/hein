...
function theme_blog($atts, $content)
{
	...
	endwhile; 
	elseif(is_search()):
		$output .= '<div class="vc_row wpb_row vc_row-fluid page_margin_top">' . sprintf(__('Es wurden keine Suchergebnisse gefunden für %s', 'pressroom'), get_query_var('s')) . '</div>';
	endif;
	$output .= '</ul>';
	if((int)$layout_type==2)
		$output .= '</div>';
