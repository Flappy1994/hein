...
function theme_mc_icon($atts)
{
	...
	return '<' . ($url!="" ? 'a' : 'span') . ($url!="" ? ' href="' . esc_attr($url) . '"' . ($url_target=="new_window" ? ' target="_blank"' : '') : '') . ($type=="social_body" || $type=="social_footer" ? ' style="background-image:url(' . get_stylesheet_directory_uri() . '/images/' . $type . '/' . $icon_color . '/' . $icon_social . '.png);"' : '') . ' class="' . ($type=="social_body" || $type=="social_footer" ? 'social_icon' : 'features_image' . ($type=="icons_small" ? ' features_image_small' : '')) . ($class!="" ? ' ' . $class : '') . ($top_margin!="none" ? ' ' . $top_margin : '') . '"' . ($title!="" ? ' title="' . $title . '"' : '') . '>' . ($type=="icons_large" || $type=="icons_small" ? '<img src="' . get_stylesheet_directory_uri() . '/images/' . $type . '/' . $icon_color . '/' . $icon_feature . '.png">' : '') . '</' . ($url!="" ? 'a' : 'span') . '>';
}
